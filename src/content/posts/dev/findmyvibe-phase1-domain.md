...
그래서 처음부터 기획 → 설계 → 구현까지 직접 하는 프로젝트를 하나 만들기로 했다.
...
2. 사용자가 답하면, Claude API가 답변을 분석해서 꼬리질문 3~5개를 동적 생성
3. 총 10~15개 질문으로 성향 프로필을 생성하고, 매칭 점수가 높은 활동 5개를 추천

백엔드는 Spring Boot + Java 25, LLM은 Claude API(Anthropic Java SDK), 프론트는 React.
...
Session이 중심이다.
"분석 시작"을 누르면 Session이 하나 생기고, 질문 → 답변 → 프로필 → 추천까지 전체 흐름이 하나의 Session 안에서 일어나는 구조.
...
`new Entity()` 대신 `Session.create()`, `Question.createBasic()` 같은 정적 메서드로만 생성한다.
UUID 자동 생성, 초기 상태 CREATED 같은 규칙을 팩토리에서 강제해서 실수를 줄이는 방식.
...
`columnDefinition = "jsonb"`를 쓰면 H2에서 테스트할 때 실패한다.
`@JdbcTypeCode(SqlTypes.JSON)`만 붙이면 Hibernate가 DB에 맞게 자동 처리하는 방식.
...
Claude Code와 함께 하는 바이브 코딩이니까, 테스트를 빡빡하게 가져가기로 했다.
...
다음부터는 인프라 코드도 테스트 먼저 작성하는 습관을 들여야겠다.
...
Spring Security를 붙이면 구현체만 교체하면 되는 구조.
...
**2. `@DataJpaTest`에서 Config가 안 잡힌다**

`@DataJpaTest`는 JPA 빈만 로드해서 `JpaAuditingConfig`가 스캔되지 않는다.
...
persist 후 바로 find하면 1차 캐시에서 원본 객체가 반환된다.
`clear()`로 캐시를 날려야 DB에서 다시 읽어온다.

```java
entityManager.persist(session);
entityManager.flush();
entityManager.clear();  // 빠뜨리면 1차 캐시에서 반환됨
...
회사에서는 "다들 이렇게 쓰니까" 하고 넘어갔던 것들 — 동작 원리, 장단점, H2 호환 같은 것들을 이번에 꽤 깊이 파봤다.
