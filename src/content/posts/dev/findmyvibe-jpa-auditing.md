...
> 도메인 레이어를 구축하면서 JPA Auditing을 적용했고, 그 과정에서 파보게 된 내용 정리.
...
설정하고 쓰기만 했지, 동작 원리나 테스트 함정은 깊이 생각해본 적이 없었음.
...
이걸 안 쓰면 Entity마다 4개 필드를 일일이 선언하고, setter 호출을 빼먹으면 null이 들어감.
...
필드 초기값(`= LocalDateTime.now()`)을 넣은 이유는 뒤에서 다룸.
...
나중에 `SecurityContextHolder`에서 꺼내는 구현체로 바꾸면 됨.
...
JPA `EntityListener`가 라이프사이클 이벤트를 감지해서 audit 필드를 채움.

- `@PrePersist` → Created/Modified 전부 채움
- `@PreUpdate` → Modified만 갱신

핵심은 **JPA 컨텍스트 안에서만 동작한다**는 점.
`new Entity()`로 만들 때는 아무 일도 안 일어나고, `entityManager.persist()`를 호출해야 리스너가 작동함.
...
JPA 컨텍스트 밖이니까 `@CreatedDate`가 동작하지 않음.
...
`@SpringBootTest`면 이런 문제가 없지만, 슬라이스 테스트 속도를 포기하고 싶지 않아서 `@Import`로 해결했음.
...
`clear()` 한 줄 차이로 테스트 신뢰성이 완전히 달라짐.
...
Hibernate가 DB 방언에 따라 자동 처리.
H2에서는 JSON 문자열, PostgreSQL에서는 jsonb로 저장됨.
...
하지만 FindMyVibe 규모에서는 문제가 되지 않음. Entity 5개뿐이고 나중에 인증 붙으면 전부 의미 있는 필드가 되는 셈.
...
마지막이 좀 재밌는데, 처음에 `answeredAt`이라는 별도 필드를 만들었다가 "결국 createdAt이랑 같은 시점 아닌가?" 하고 지웠음.
...
다음에는 Flyway 마이그레이션과 ArchUnit을 마무리하고, Phase 1 나머지 이야기를 정리할 예정.
